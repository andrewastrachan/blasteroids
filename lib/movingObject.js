(function () {
  ...
  MovingObject.prototype.relocate = function () {
	...
	MovingObject.prototype.fireBullet = function() {
		...
		var bullet = new Asteroids.Bullet({game: this.game, vel: [newVel[0], newVel[1]], pos: this.bulletPos()});
		this.gunRecoil(.05);
	...
	MovingObject.prototype.fireCannon = function() {
		if (!this.cannonReloading) {
			var addedVel = Asteroids.Util.itemVector(this.angle);
			var newVel = [(addedVel[0] * 7), (addedVel[1] * 7)];
			var cannonBall = new Asteroids.Bullet({game: this.game, vel: [newVel[0], newVel[1]], pos: this.bulletPos(), radius: "20", color: "orange", type: "indestructable"});
			this.gunRecoil(4);
